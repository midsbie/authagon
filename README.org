...
For a full working app that uses =SessionCtl=, see [[examples/oauth2-web-starter][examples/oauth2-web-starter]].
For an example where the application manages its own sessions without =SessionCtl=, see
[[examples/oauth2-core-flow][examples/oauth2-core-flow]].
...
- Sessions:
  - =SessionCtl= is an optional, batteries-included helper that stores =AuthResult= in a
    =SessionStorer[AuthResult]= behind a browser cookie.
  - Applications with existing session infrastructure can ignore =SessionCtl= and manage app
    sessions directly; see [[examples/oauth2-core-flow][examples/oauth2-core-flow]] for a minimal example.
